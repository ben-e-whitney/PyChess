...
class EmailForm:
    ...
    def _on_okay(self):
        ...
        text += '\tHOST = {ip}\tPORT = {port}'.format(ip=self.external_ip,
            port=self.port_number.get())
        part.set_payload(text)
        Encoders.encode_base64(part)
        part.add_header('Content-Disposition',
                        'attachment; filename="GameClient.py"')
        msg.attach(part)
    ...
    def _refresh_ui(self):
        ...
        Label(self.root, text='To').grid(row=0, sticky=W, pady=(20, 0),
                                         padx=(50, 0))
        Label(self.root, text='Gmail address').grid(row=2, sticky=W,
                                                    padx=(50, 0))
        ...
        e1.grid(row=0, column=1, sticky=E, columnspan=4, pady=(20, 0),
                padx=(0, 50))
        ...
        ok_btn     = Button(self.root, text='Send', command=self._on_okay,
                            width=5).grid(row=5, column=2, columnspan=2,
                                          sticky=E, pady=(20, 20))
        cancel_btn = Button(self.root, text='Cancel').grid(row=5,
            sticky=E+W, column=4, columnspan=2, padx=(0, 50), pady=(20, 20))
...
class ChessMenu:
    ...
    def _refresh_ui(self):
        ...
        Label(self.root, text='Name').grid(row=0, sticky=W, pady=(50, 0),
                                           padx=(50, 0))
        Label(self.root, text='Opponent').grid(row=2, sticky=W, padx=(50, 0))

        mail_btn = Button(text='Invite', master=self.root,
                          command=self._on_mail_click)
        ...
        e2.config(width=3)
        e1.grid(row=0, column=1, sticky=W, columnspan=2, pady=(50, 0),
                padx=(0, 50))
        e2.grid(row=2, column=1, sticky=W+E, padx=(0, 1))
        mail_btn.grid(row=2, column=2, sticky=E+W, padx=(0, 50))
        play_btn.grid(row=3, column=0, sticky=E+W, columnspan=3, pady=(25, 20),
                      padx=(50, 50))
